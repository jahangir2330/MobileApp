...
namespace AppConfig.HelperClasses
{
    public class clsPrivateKeys
    {
        ...
        #region jahangir
        public long DecodeAfterQuestionMarkAndParam(string ParamName, string textlikeparamsiquealblablabla)
        {
            long returnValue = -99;
            try
            {
                string onlytext = textlikeparamsiquealblablabla.Replace("?params=", "");
                returnValue = Convert.ToInt64(DecryptFromOnlyText(ParamName, onlytext));
                return returnValue;
            }
            catch (Exception)
            {
        ...
        public long DecodeFromFullUrl(string ParamName, string textlikeparamsiquealblablabla)
        {
            long returnValue = -99;
            try
            {
                var url = textlikeparamsiquealblablabla;
                string onlytext = HttpUtility.UrlEncode(url.Split('?')[1].Replace("params=", ""));
                returnValue = Convert.ToInt64(DecryptFromOnlyText(ParamName, onlytext));
                return returnValue;
            }
            catch (Exception)
        ...
        public string DecryptFromOnlyText(string ParamName, string onlytext)
        {
            EncryptionHelper objEnc = new EncryptionHelper();
            string result = string.Empty;
            if (string.IsNullOrEmpty(onlytext)) { return string.Empty; }
            System.Collections.Specialized.NameValueCollection paramList = System.Web.HttpUtility.ParseQueryString(objEnc.Decrypt(System.Text.Encoding.UTF8.GetString(System.Web.HttpUtility.UrlDecodeToBytes(onlytext)), true, _pk));
